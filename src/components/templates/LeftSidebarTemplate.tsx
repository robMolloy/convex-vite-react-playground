...
const SidebarButtonWrapper = (p: {
  ...
  return p.href ? (
    <Link to={p.disabled ? "#" : p.href} className={p.disabled ? "pointer-events-none" : ""}>
      {p.children}
    </Link>
  ) : (
    p.children
    ...
    <SidebarButtonWrapper href={p.href} disabled={p.disabled}>
      <PossibleTooltipWrapper tooltipContent={p.tooltipContent}>
        <Button
          ...
          className={`w-full justify-between ${p.disabled ? "pointer-events-none" : ""}`}
          onClick={p.onClick}
          disabled={p.disabled}
        >
...
export const LeftSidebarTemplate = (p: {
  ...
  return (
    <PreserveScrollAbility className="border-r">
      ...
      {p.bottom && <div className="flex flex-col gap-1 border-t p-2">{p.bottom}</div>}
